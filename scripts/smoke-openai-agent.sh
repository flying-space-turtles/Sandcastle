...
if [[ "${SANDCASTLE_ALLOW_REAL_MODEL:-0}" != "1" ]]; then
    ...
    echo "Set SANDCASTLE_ALLOW_REAL_MODEL=1 and a cost limit of at most 0.10." >&2
    exit 2
fi

max_cost="${ARENA_AGENT_MAX_COST_USD_PER_CALL:-0.10}"
awk -v cost="${max_cost}" 'BEGIN { exit !(cost <= 0.10) }' || {
    echo "ARENA_AGENT_MAX_COST_USD_PER_CALL must be at most 0.10" >&2
    exit 2
}
