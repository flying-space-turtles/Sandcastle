...
grep -Fq 'host.docker.internal:172.30.0.1' <<< "${visualizer_block}"
grep -Fq 'subnet: 172.30.0.0/24' "${deterministic_fixture}/docker-compose.yml"
grep -Fq 'gateway: 172.30.0.1' "${deterministic_fixture}/docker-compose.yml"
grep -Fq 'location /bot-api/' "${ROOT}/visualizer/nginx.conf"
grep -Fq 'location /firewall-ws' "${ROOT}/visualizer/nginx.conf"
grep -Fq 'proxy_pass http://172.30.0.1:6789;' "${ROOT}/visualizer/nginx.conf"
