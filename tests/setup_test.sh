...
bot_controller_block="$(sed -n '/^  bot-controller:/,/^  visualizer:/p' "${deterministic_fixture}/docker-compose.yml")"
grep -Fq 'control-plane: {}' <<< "${bot_controller_block}"
visualizer_block="$(sed -n '/^  visualizer:/,/^volumes:/p' "${deterministic_fixture}/docker-compose.yml")"
grep -Fq 'network_mode: host' <<< "${visualizer_block}"
grep -Fq 'location /bot-api/' "${ROOT}/visualizer/nginx.conf"
grep -Fq 'location /firewall-ws' "${ROOT}/visualizer/nginx.conf"
grep -Fq 'listen 127.0.0.1:4173;' "${ROOT}/visualizer/nginx.conf"
grep -Fq 'proxy_pass http://127.0.0.1:8000/api/;' "${ROOT}/visualizer/nginx.conf"
grep -Fq 'proxy_pass http://127.0.0.1:7878/;' "${ROOT}/visualizer/nginx.conf"
grep -Fq 'proxy_pass http://127.0.0.1:6789;' "${ROOT}/visualizer/nginx.conf"
